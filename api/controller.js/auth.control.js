...
export const signin = async(req,res,next)=>{
    ...
    try{
        ...
        if(!validUser) return next(errorHandler(400, 'User not found,Is it your first time here broo'));
        const validPassword = bcryptjs.compareSync(password, validUser.password);
        if(!validPassword) return next(errorHandler(401,'Wrong Credentials broo'));
